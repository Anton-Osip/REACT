...
describe('RefreshIcon', () => {
  ...
  it('renders svg paths inside the icon', () => {
    const { container } = render(<RefreshIcon size={20} />);
  ...
  it('exposes decorative wrapper for assistive tech', () => {
    render(<RefreshIcon size={20} />);

    expect(screen.getByRole('img', { hidden: true })).toBeInTheDocument();
  });

  it('applies size to the wrapper', () => {
    render(<RefreshIcon size={32} />);
  ...
  it('forwards svgProps to the svg element', () => {
    const { container } = render(
      <RefreshIcon svgProps={{ id: 'refresh-svg-forward-test' }} />
    );

    expect(container.querySelector('svg')).toHaveAttribute(
      'id',
      'refresh-svg-forward-test'
    );
  });
});
